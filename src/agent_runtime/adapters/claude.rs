    ...
    pub(crate) fn new() -> Self {
        Self {
            capabilities: AgentCapabilities {
                ...
                skills: vec![
                    "code_generation".into(), "refactor".into(), "review".into(),
                    "debug".into(), "test".into(), "explain".into(),
                ],
                long_running: true,
...
impl AgentInterface for ClaudeAdapter {
    ...
    fn spawn(&self, config: &AgentConfig) -> Result<AgentHandle, String> {
        let program = config.command.as_deref().unwrap_or("claude").to_string();
    ...
    fn shutdown(&self, handle: &AgentHandle) -> Result<(), String> {
        log::info!("claude adapter shutdown: id={}", handle.id);
        let _ = pty_common::write_to_pty(handle, "/exit");
        pty_common::kill_pty(handle)
    }

    fn tool_type(&self) -> &str { "claude" }
}
