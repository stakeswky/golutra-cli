...
impl CodexAdapter {
    pub(crate) fn new() -> Self {
        Self {
            capabilities: AgentCapabilities {
                languages: vec!["python".into(), "typescript".into(), "javascript".into()],
...
impl AgentInterface for CodexAdapter {
    ...
    fn spawn(&self, config: &AgentConfig) -> Result<AgentHandle, String> {
        let program = config.command.as_deref().unwrap_or("codex").to_string();
    ...
    fn shutdown(&self, handle: &AgentHandle) -> Result<(), String> {
        log::info!("codex adapter shutdown: id={}", handle.id);
        pty_common::kill_pty(handle)
    }

    fn tool_type(&self) -> &str { "codex" }
}
