...
impl GeminiAdapter {
    pub(crate) fn new() -> Self {
        Self {
            capabilities: AgentCapabilities {
                languages: vec![
                    "python".into(), "typescript".into(), "javascript".into(),
                    "go".into(), "java".into(), "kotlin".into(),
                ],
                skills: vec![
                    "code_generation".into(), "explain".into(), "review".into(),
                    "test".into(),
                ],
                long_running: true,
...
impl AgentInterface for GeminiAdapter {
    ...
    fn spawn(&self, config: &AgentConfig) -> Result<AgentHandle, String> {
        let program = config.command.as_deref().unwrap_or("gemini").to_string();
    ...
    fn shutdown(&self, handle: &AgentHandle) -> Result<(), String> {
        log::info!("gemini adapter shutdown: id={}", handle.id);
        pty_common::kill_pty(handle)
    }

    fn tool_type(&self) -> &str { "gemini" }
}
