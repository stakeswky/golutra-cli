...
impl QwenAdapter {
    pub(crate) fn new() -> Self {
        Self {
            capabilities: AgentCapabilities {
                languages: vec!["python".into(), "typescript".into(), "java".into()],
...
impl AgentInterface for QwenAdapter {
    fn id(&self) -> &str { "qwen-code" }
    fn capabilities(&self) -> &AgentCapabilities { &self.capabilities }

    fn spawn(&self, config: &AgentConfig) -> Result<AgentHandle, String> {
        let program = config.command.as_deref().unwrap_or("qwen").to_string();
    ...
    fn shutdown(&self, handle: &AgentHandle) -> Result<(), String> {
        log::info!("qwen adapter shutdown: id={}", handle.id);
        pty_common::kill_pty(handle)
    }

    fn tool_type(&self) -> &str { "qwen" }
}
